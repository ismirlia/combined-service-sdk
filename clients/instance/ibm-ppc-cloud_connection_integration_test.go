...
func TestCloudConnection(t *testing.T) {
	...
	cloudConnectionClient := client.NewIBMPPCCloudConnectionClient(context.Background(), session, utl.CloudInstanceID)
	jobClient := client.NewIBMPPCJobClient(context.Background(), session, utl.CloudInstanceID)
