...
func Test_crnBuilder(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{"12345", "dal12", "dal.ppc-aas.cloud.ibm.com"},
			want: "crn:v1:bluemix:public:ppc-aas:dal12:a/12345:%s::",
		},
		{
			name: "Generate for Staging",
			args: args{"12345", "dal12", "dal.ppc-aas.test.cloud.ibm.com"},
			want: "crn:v1:staging:public:ppc-aas:dal12:a/12345:%s::",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func Test_getSPClient(t *testing.T) {
	type args struct {
		debug  bool
		host   string
		scheme string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := getSPClient(tt.args.debug, tt.args.host, tt.args.scheme); got == nil {
				t.Errorf("getSPClient() = %v", got)
			}
		})
	}
}
